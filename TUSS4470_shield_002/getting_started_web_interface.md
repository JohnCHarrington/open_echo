...
Run the following command, adding your `SERIAL_PORT` (e.g. /dev/ttyACM0) and setting `MEDIUM` to water/air. To test the interface without data, use SERIAL_PORT=test. You can also set `NUM_SAMPLES` and `BAUD_RATE` if required. Note that these settings are not sent to the arduino - they need to match what is on the board to get the correct results!

```bash
SERIAL_PORT=<your_serial_port> MEDIUM=water uv run uvicorn --host=0.0.0.0 app:app
```
Then go to http://localhost:8000 to see your echoes.
